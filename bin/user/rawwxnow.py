...
class RAWWXNOW(StdService):

    def __init__(self, engine, config_dict):
        super(RAWWXNOW, self).__init__(engine, config_dict)
        loginf("service version is %s" % VERSION)
        d = config_dict.get('RAWWXNOW', {})
        s = config_dict.get('Station', {})
        self.filename = d.get('filename', '/var/tmp/wxnow.pkt')
        self.latitude = float(s.get('latitude'))
        self.longitude = float(s.get('longitude'))
    ...
    def calculate(self, packet, archive):
        ...
        latitude = weeutil.weeutil.latlon_string(self.latitude, ('N', 'S'), 'lat')
        longitude = weeutil.weeutil.latlon_string(self.longitude, ('E', 'W'), 'lon')
        data['latlon'] = '%s%s%s/%s%s%s_' % (latitude + longitude)
        ...
        data['luminosity'] = nullproof('radiation', packet)
        return data

    def write_data(self, data):
        fields = []
        fields.append(data['latlon'])
        ...
        if data['luminosity'] is None:
            data['luminosity'] = ""
        elif data['luminosity'] < 999.5:
            data['luminosity'] = "L%03d" % int(data['luminosity'] + 0.5)
        elif data['luminosity'] < 1999.5:
            data['luminosity'] = "l%03d" % int(data['luminosity'] - 1000 + 0.5)
        else:
            data['luminosity'] = ""
        fields.append(data['luminosity'])
